...
~1,510 rows → small dataset

~8 - 12 continuous inputs

Macroeconomic & health data → mostly smooth relationships

Regression target → outputs are continuous

Implications:

1 hidden layer: Can fit smooth nonlinear functions, but limited flexibility for multiple interacting nonlinearities.

2 hidden layers: Good for moderate interactions and curvature. Often enough for tabular regression.

3 hidden layers: Adds extra capacity to model subtle nonlinear interactions (e.g., real_gdp * population affecting breast cancer rate) without being overly deep.

Key: Because we added BatchNorm + Dropout, the network can afford a small extra layer without overfitting.
"""